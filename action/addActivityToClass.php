...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("
            INSERT INTO assessment_author (instructor_courseID, assessment_type, assessment_refID, assessment_time, upload_date) 
            VALUES (?, ?, ?, ?, NOW())
        ");
        ...
        if ($stmt->execute()) {
            ...
            if ($studentsResult->num_rows === 0) {
                echo "<script>
                    alert('Activity uploaded, but no students are currently enrolled in this class. They will be assigned automatically once enrolled.');
